...
Download the folder "demo3*zip" on your system and extract it in any local folder (\*\_to be changed based on the method used in the sections below*\*). The zip file contains the model (.osm), user data (.csv) and weather file (.epw) to follow along the demo.
...
To do that, we will need to use the _*userdata_space.csv*_. In the `demo3` zip file, there is a `user_data` folder. Inside this folder, we have a `userdata_space.csv` file. Let's open the file to inspect.
...
3. **Execute:** Open Command Prompt as an administrator and type the following command with paths to the openstudio\bin and the OSW directories.
   ...
   ```ruby
   C:\openstudio-3.7.0\bin\openstudio.exe  run -w "C:\Users\sample_user\demo3\test.osw"
   ```
   ...
   - _*demo3*_ to be replaced by the folder path where test.osw is located.
     {{% /notice %}}
...
- _*Baseline Value: The values assigned to the generated baseline model to match the Standard Requirements (Please note the units for LPD are different)*_
